...
def _validate_entry(errors: list, start_line: int, full_text: str, id_pattern: re.Pattern, current_id: str = None):
    ...
    # Правило:
    # 1) Если в тексте есть символ ", то весь OriginalText должен быть заключён в двойные кавычки.
    # 2) Внутренние кавычки должны образовывать пары "" и давать целое число «цитат»:
    #    после вычитания внешней пары кавычек количество внутренних кавычек должно быть кратно 4
    #    (каждая цитата — это "" + "" -> 4 кавычки).
    if '"' in text:
        if not (len(text) >= 2 and text.startswith('"') and text.endswith('"')):
            # Это считаем фатальной ошибкой
            errors.append(
                f"❌ Строка {start_line}, ID: {display_id}: Некорректное использование кавычек. "
                f'Если текст содержит символ " (двойная кавычка), весь OriginalText должен быть заключён в кавычки. '
                f"Начало текста: '{text[:100]}'"
            )
        else:
            quote_count = text.count('"')
            # Уже знаем, что как минимум 2 кавычки (внешняя пара)
            inner_quote_count = quote_count - 2
            if inner_quote_count % 4 != 0:
                # А это только предупреждение, не фаталка
                errors.append(
                    f"⚠️ Строка {start_line}, ID: {display_id}: Похоже на сломанные кавычки/тег. "
                    f"После внешней пары кавычек количество внутренних кавычек должно быть кратно 4 "
                    f"(каждая пара \"\" открывает/закрывает цитату). Всего кавычек: {quote_count}. "
                    f"Начало текста: '{text[:100]}'"
                )
